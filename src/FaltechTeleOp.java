...
public class FaltechTeleOp extends OpMode {
    ...
    void doCollector() {
        ...
        if (intake) robot.glyphCollector.collectionIntake(1);
        else if (eject) robot.glyphCollector.collectionExpel(1);
        else if (rotate > 0) robot.glyphCollector.collectionRotate(rotate);
        else robot.glyphCollector.collectionIntake(0);

        boolean elevatorUp = gamepad2.right_bumper;
        boolean elevatorDown = gamepad2.left_bumper;

        if (elevatorUp) robot.glyphCollector.elevatorUp(0.8);
        else if (elevatorDown) robot.glyphCollector.elevatorUp(-0.8);
        else if (gamepad2.y)
            robot.glyphCollector.elevatorUp(0.16);  // TODO: What is this creeper intake mode?
        else robot.glyphCollector.elevatorUp(0);
    }
